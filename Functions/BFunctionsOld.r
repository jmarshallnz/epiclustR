...
BInitialise <- function() {
  if (Bmode==0) {# Rectangular array ur x sdi
    ...
    sdi<<-matrix(0,1,mbs)
    for (i in 1:mbs) {
      sdi[i]<<-length(which(thresholds<=input[i]))
    }
    sdi<<-1-min(sdi)+sdi
    sds<<-max(sdi)
    Bmax<<-sds
    ...
    for (i in 1:mbs) {
      input[i]<-length(which(thresholds<=input[i]))
    }
    sdi[which(ur<7)]<<-10+input[which(ur<7)]
    sds<<-max(sdi)
    Bmax<<-sds
    ...
    BRank<<-sds-2
    Blink<<-sdi
  } else if (Bmode==12) {# single line for SDI
    Bmax<<-sds
    for (i in 1:sds) {
      cat("Group",i,"has",length(which(sdi==i)),"members.\n")
    }
...
BUpdate <- function(i=0) {
  ...
  kB<<-rgamma(1,aB+BRank/2,rate=(bB+BSumFunction()/2))
  # Update B
  for (j in 1:Bmax) {
    wch<-Bn[[j]]
    if (i%%2==0) {
      proposal<-rnorm(1,mean(B[wch]),(1/kB/length(wch))^(0.5))
      ap<-BLikelihood(j,proposal,which(Blink==j))
    } else {
      proposal<-rnorm(1,B[j],sd=sigmaB)
      ap<-exp(-kB*sum((B[wch]-proposal)^2-(B[wch]-B[j])^2)/2)*BLikelihood(j,proposal,which(Blink==j))
    }
    un<-runif(1)
    if (un<=ap) {
      B[j]<<-proposal
...
BSample <- function() {
  ...
  cat(kB,file="kB.txt",append=TRUE,sep="\n")
...
BAnalysis <- function() {
  ...
  if (Bmode==0 || Bmode==1) {
    ...
    plot(1:10,t="n",xlab="SDI",ylab="B",main="SDI Analysis",xlim=c(1,10),ylim=c(-m,m))
    lines(1:10,B[1:10])
    if (Bmode==7) {titl<-"Chicken Density"}
    if (Bmode==8) {titl<-"Sheep Density"}
    if (Bmode==9) {titl<-"Cow Density"}
    if (Bmode==10) {titl<-"Beef Density"}
    if (Bmode==11) {titl<-"Dairy Density"}
    ax<-1:(sds-10)
